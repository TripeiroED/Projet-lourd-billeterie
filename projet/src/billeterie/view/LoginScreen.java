...
public class LoginScreen {
    ...
    public LoginScreen(App app) {
        ...
        btnLogin.setOnAction(e -> {
            ...
            try {
                if (app.getUserController() == null) {
                    message.setStyle("-fx-text-fill: #ff4c4c;");
                    message.setText("Erreur serveur : pas de connexion DB (verifie le driver MySQL)");
                    System.err.println("UserController null: la DB n’est pas connectée.");
                    return;
                }

                User loggedUser = app.getUserController().authenticate(user, pass);
                ...
                message.setStyle("-fx-text-fill: #ff4c4c;");
                message.setText("Erreur serveur : " + ex.getMessage());
                ex.printStackTrace();
            } catch (Exception ex) {
                message.setStyle("-fx-text-fill: #ff4c4c;");
                message.setText("Erreur inattendue : " + ex.getMessage());
                ex.printStackTrace();
            }
        });
